...
if is_record:
    env = wrappers.Monitor(
        env,
        'RL_movies/cartpole-experiment-3',
        force=True,
...
init_stddev_2 = 0.18 / np.sqrt(
    hidden_size)  # 0.2 0.18 # second layer initialisation dist
...
class NN():
    ...
    def anchor(self, sess, regularise=False):
        ...
        loss_anchor += lambda_anchor[1] * tf.reduce_sum(
...
for run in range(0, n_runs):
    ...
    for episode in range(0, n_episodes):
        ...
        ens_i = np.random.randint(0,
                                  n_ensemble)  # select random NN from ensemble
        total_reward = 0  # per episode
        explore = 0
        observation = env.reset()

        # test if 'solved' - need 100 episodes with avg >= 195
        if len(reward_list) > 100 and np.sum(
                reward_list[-100:]) / 100 >= 195. and print_win:
            print('\n-- LEVEL 1 RL COMPLETE --')
        ...
        e = min(
            eps_max,
            max(
                eps_min, eps_max - (eps_max - eps_min) * (episode - n_burn_in) /
                (eps_decay - n_burn_in)))
        ...
        for i in range(max_episode_len):
            ...
            if n_ensemble == 1:
                ...
                for j in range(0, n_ensemble):
                    feed[NNs[j].inputs] = observation.reshape(
                        (1, *observation.shape))
                ...
                std_diff = np.sqrt(
                    np.square(Qs_ens_std[0]) + np.square(Qs_ens_std[1]))
            ...
            if done:
                ...
                # print info
                print('Run: {}'.format(run), 'Ep: {}'.format(episode),
                      'Rew: {}'.format(int(total_reward)),
                      'L: {:.3f}'.format(l), 'e: {:.3f}'.format(e))
                # env.reset()
                # print('timestep:',i)
                ...
                ind_focus = np.argpartition(episode_exp[:, 4],
                                            -n_large_limit)[-n_large_limit:]
                ...
                if len(experience_replay) > 100000:
                    ...
                    del experience_replay[del_i]
                experience_replay.append([
                    observation, action, reward, observation_1, var_action[-1]
                ])
            ...
            if episode > n_burn_in and consecutive_wins < 10:  #and episode < eps_decay: # try override for trained model
                ...
                samples_i = np.random.choice(
                    range(len(experience_replay)),
                    size=batch_size - n_large_limit,
                    replace=True)
                samples = []
                ...
                episode_ends = (replay_observations_ == np.zeros(
                    replay_observations[0].shape)).all(axis=1)
                ...
                for _ in range(n_train_reps):  # repeat training multiple times
                    ...
                    targets = replay_rewards / 20. + gamma * np.max(
                        target_qs, axis=2)
...
if view_quality:
    ...
    runs_explore_mean = runs_explore_batch.mean(
        axis=0) / runs_mean * 200  # divide by run length
    ...
    ax.grid()
    ax.fill(
        np.concatenate([x_batch, x_batch[::-1]]),
        np.concatenate(
            [runs_mean - 2 * runs_std, (runs_mean + 2 * runs_std)[::-1]]),
        alpha=1.0,
        fc=col2,
        ec='None',
        label='2 std devs')
    ...
    if plot_compare:
        runs_reward_batch_comp = pickle.load(
            open("RL_outputs/pickles/" + compare_to, "rb"))
...
if save_results:
    pickle.dump(runs_reward_batch,
                open("RL_outputs/pickles/" + get_time() + ".p", "wb"))

solved_mean = np.average(runs_solved) - 100
solved_std = np.std(runs_solved)
solved_se = solved_std / np.sqrt(n_runs)
print('\nEpisodes required before solve, averaged over', n_runs, 'runs:',
      np.round(solved_mean, 2), '+/-', 2 * np.round(solved_se, 2))
